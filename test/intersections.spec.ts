...
describe('Intersections', () => {
  ...
  it('Basic intersection with generic', () => {
    interface GenericBase<T> {
      a: number;
      b: T;
    }

    interface Nested1Interface {
      nested1: string;
      nested2: string;
    }
    interface Nested2Interface {
      nested3: string;
      nested4: string;
    }

    type NestedInterface = Nested1Interface | Nested2Interface;

    type NestedType = Partial<NestedInterface> & { id: string };

    expect(v.validate({ a: 1, b: { nested3: 'string', id: '1' } }, convertToSchema<GenericBase<NestedType>>())).toBe(true);

    expect(v.validate({ a: 1, b: 2 }, convertToSchema<GenericBase<NestedType>>())).toBeInstanceOf(Array);
    expect(v.validate({ b: { nested1: 'string', id: '1' } }, convertToSchema<GenericBase<NestedType>>())).toBeInstanceOf(Array);
    expect(v.validate({ a: 1 }, convertToSchema<GenericBase<NestedType>>())).toBeInstanceOf(Array);
  });
